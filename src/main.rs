    ...
    F: FnMut() -> T,
{
 ...
 fn generate_dummy_record_batch(schema: &Schema) -> RecordBatch {
    let columns = schema
        .fields()
        .iter()
        .map(|_| Arc::new(Int32Array::from(vec![Some(42);20])) as Arc<dyn arrow_array::Array>)
        .collect::<Vec<_>>();
    RecordBatch::try_new(Arc::new(schema.clone()), columns).unwrap()
 }
 
 fn test_write_read_parquet_col(fields_num: u32) -> Result<(Duration, Duration, Duration)> {
    let schema = generate_column_schema(fields_num);let batch = generate_dummy_record_batch(&schema);
    ...
    println!("Approach 1: average time elapsed over 10 runs: {:?}", avg_time_1);
    ...
    println!("Approach 2: average time elapsed over 10 runs: {:?}", avg_time_2);
    thread::sleep(Duration::from_secs(5));
    ...
    println!("Approach 3: average time elapsed over 10 runs: {:?}", avg_time_3);
 ...
 fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut wtr = csv::Writer::from_path("parquet_metadata_benchmark.csv")?;
    wtr.write_record(&[
        "fields_num",
        "avg_time_thrift_s",
    ...
    for &fields_num in &field_counts {
        println!("Testing with {} columns...", fields_num);
        let (avg_time_1, avg_time_2, avg_time_3) = test_write_read_parquet_col(fields_num)?;
        wtr.write_record(&[
